...
MyLine::MyLine(const cv::Point2f &_start, const cv::Point2f &_end) : start{_start}, end{_end} {
    this->length = MyLine::lineDist(*this);
    this->slope = MyLine::slopeFactor(*this);
...
bool MyLine::areLinesEqual(const MyLine &thisLine, const MyLine &other) {
    ...
    if (std::fabs(product / (thisLine.length * other.length)) < cos(CV_PI / 30))
        return false;
...
float MyLine::slopeFactor(const MyLine &myLine) {
    ...
    return ((myLine.end.x - myLine.start.x) == 0) ? 100 :
